...
- **Perceptrons**:
  ...
  - Then $\arg\max_{w} \log \prod_i P(y_i \mid x_i; w) = \arg\min_{w} \sum_i \tfrac{1}{2}(y_i - f(x_i; w))^2$
    - We can find $\arg\max_{w} \log \prod_i P(y_i \mid x_i; w)$ by iterating: $$w_d = w_d + \eta \sum_i (y_i - o_i) \, o_i (1 - o_i) \, x_{id}$$
...
- If there were, then there would be constants $w_1$ and $w_2$ such that:
  ...
  - When $$x_1 \ne x_2$$, then $\sigma(w_1 x_1 + w_2 x_2) \geq \theta$
- Case 1: $$x_1 = 1, x_2 = 0$$
...
- By applying the chain rule, and working in reverse order, we get: $$\frac{\partial f_n}{\partial x}
